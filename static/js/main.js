// ==========================================================================
//  main.js - エリたま編成ジェネレーター メインJavaScript
//  注意: このファイルより先にindex.htmlでJinja2変数が定義されている必要があります
//  - ALL_ALIENS, ALIEN_SKILL_DATA, ALL_EFFECTS, S_SKILL_EFFECTS, ALIEN_EFFECTS
// ==========================================================================
 ...
 * 管理モードUIの更新
 */
function updateAdminUI(skipRender = false) {
    ...
    // パーティスロットを再描画して管理ボタンの状態を更新（skipRender=falseの場合のみ）
    if (!skipRender) {
        renderAllParties();
    }
                ...
                if (data.success) {
                    ...
                    if (overlay && container && content) {
                        ...
                        document.getElementById('dict-update-close').addEventListener('click', () => {
                            overlay.style.display = 'none';
                            modal.remove();
                            updateAdminUI(true); // pendingChangesの表示のみ更新（再描画スキップ）
                        }, { once: true });
                    } else {
